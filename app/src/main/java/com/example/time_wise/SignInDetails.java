...
public class SignInDetails extends AppCompatActivity {
    ...
    private void initViews() {
        ...
        // הגדרת TimePicker לכל שדות הזמן
    ...
    private void updateUI() {
        // עדכון כותרת היום הנוכחי
        txtDayTitle.setText(days[currentDayIndex].name());

        // ביום האחרון (שבת) משנים את הטקסט בכפתור
    ...
    private void handleNextStep() {
        // בדיקה אם הכל ריק
        if (isAllEmpty()) {
            Toast.makeText(this, "Please fill at least one time slot", Toast.LENGTH_SHORT).show();
            return;
        }

        // שמירת הנתונים של היום הנוכחי לרשימות
        collectCurrentDayData();

        if (currentDayIndex < days.length - 1) {
            // מעבר ליום הבא באותו מסך
            currentDayIndex++;
            clearAllFields();
            updateUI();
        } else {
            // סיום התהליך ושמירה ל-Firebase
            saveDataToFirebase();
        }
    }
    ...
    private boolean isRangeEmpty(EditText start, EditText end) {
        return start.getText().toString().isEmpty() || end.getText().toString().isEmpty();
    }

    private boolean isAllEmpty() {
        return isRangeEmpty(EffStart, EffEnd) && isRangeEmpty(BadStart, BadEnd) && isRangeEmpty(SleepStart, SleepEnd);
    }

    private void clearAllFields() {
        EffStart.setText(""); EffEnd.setText("");
        BadStart.setText(""); BadEnd.setText("");
        SleepStart.setText(""); SleepEnd.setText("");
    }
